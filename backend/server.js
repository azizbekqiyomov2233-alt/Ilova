 ...
 * - Video/poster upload (max 700 MB) -> diskka saqlanadi
...
function publicFileUrl(req, filename) {
  ...
  return `${proto}://${host}/videos/${filename}`;
}

async function saveUploadMeta(file, url) {
  try {
    await pool.query(
...
function handleUpload(fieldName, missingText) {
  return (req, res) => {
    upload.single(fieldName)(req, res, async (err) => {
      ...
      if (!req.file) return res.status(400).json({ error: missingText });

      const url = publicFileUrl(req, req.file.filename);
      await saveUploadMeta(req.file, url);
...
// ---------- VIDEO / POSTER UPLOAD ----------
app.post('/api/upload-video', handleUpload('video', 'Video fayl yo\'q'));
app.post('/api/upload/video', handleUpload('video', 'Video fayl yo\'q'));
app.post('/api/upload-poster', handleUpload('poster', 'Poster fayl yo\'q'));
app.post('/api/upload/poster', handleUpload('poster', 'Poster fayl yo\'q'));
